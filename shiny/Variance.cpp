...
double pLogDens (vec y, double sigmaSq, double palpha, double pbeta) {
  int n = y.n_elem;
  double density = -(1+palpha) * log(sigmaSq) - pbeta / sigmaSq;
  for(int i = 0; i < n; ++i){
...
double qigamma (double x, double alpha, double beta){
  boost::math::gamma_distribution<> G(exp(alpha), exp(-beta));
  return 1.0 / quantile(G, 1-x);
}

double ELBO (vec y, double par1, double par2, double palpha, double pbeta, bool lognormal, int n = 100){
  double elbo = 0;
  vec epsilon;
  double sigmaSq;
  if(lognormal){
  ...
  for(int i = 0; i < n; ++i){
    ...
    elbo += pLogDens(y, sigmaSq, palpha, pbeta) - qLogDens(sigmaSq, par1, par2, lognormal);
  }
  return elbo/n;
}

vec deriv (vec y, double par1, double par2, double epsilon, double palpha, double pbeta, bool lognormal) {
  int N = y.n_elem;
  if(lognormal){
    ...
    double sigmaSq = qigamma(epsilon, par1, par2);
    double h = 0.00000001;
    double par1h = par1 + h;
    double par2h = par2 + h;
    double sigmaSq1 = qigamma(epsilon, par1h, par2);
    double sigmaSq2 = qigamma(epsilon, par1, par2h);
    double dpds = -(0.5*N+1+palpha) * pow(sigmaSq, -1) + pbeta/pow(sigmaSq, 2);
    for(int i = 0; i < N; ++i){
...
//[[Rcpp::export]]
Rcpp::List SGA_Var (vec y, bool lognormal, double initPar1, double initPar2, double palpha=1, double pbeta=1,
                      int M=50, int maxIter=5000, double threshold=0.01, double alpha=0.1, bool adam=true) {
  int T = y.n_elem;
  double beta1 = 0.9;
  double beta2 = 0.999;
  double e = 0.0000001;
  ...
  int iter = 0;
  vec LB(maxIter+1);
  LB[0] = ELBO(y, params[0], params[1], palpha, pbeta, lognormal);
  double diff = 1;
  while(diff > threshold | iter < 10){
    ...
    for(int i = 0; i < M; ++i){
      vec p = deriv(y, params[0], params[1], epsilon[i], palpha, pbeta, lognormal);
      partials += p/M;
      pSq += pow(p, 2)/M;
    }
    if(adam){
    ...
    LB[iter] = ELBO(y, params[0], params[1], palpha, pbeta, lognormal);
