...
// [[Rcpp::export]]
Rcpp::List carsTest (mat x, Rcpp::NumericMatrix lambdaIn, vec hyperParams, int S = 1, int N = 100, int maxIter = 1, 
                     double alpha = 0.01, double threshold = 0.01){
  ...
  int T = x.n_rows;
  vec logPeval(S);
  Matrix<double, Dynamic, 1> gradientJ(24), tempGrad(24);
  MatrixXd gradientP(24, S);
  double logJeval, logQeval;
  vec meanGradient(24), Mt(24), Vt(24), LB(maxIter);
  mat epsilon(12, S), theta(12, S);
  
  LB.fill(0); Mt.fill(0); Vt.fill(0);
  
  mat sobol = sobol_points(100+S, 12);
  // Standard normal density to transform [0,1] numbers to standard normal
  boost::math::normal_distribution<> epsDist(0, 1);
  double diff = threshold + 1, meanLB = 0, omeanLB;
  int iter = 0;
  ...
  while(diff > threshold){
    ...
    for(int s = 0; s < S; ++s){
      ...
      for(int i = 0; i < 12; ++i){
        ...
        if(unif(100+s, i) > 0.9995){
          epsilon(i, s) = 3.290527;
        } else if (unif(100+s, i) < 0.0005) {
          epsilon(i, s) = -3.290527;
        } else {
          epsilon(i, s) = quantile(epsDist, unif(100+s, i));
        }
        logQeval += - 0.91894 -  pow(epsilon(i, s), 2) / 2;
      }
      ...
      // update estimates of deriv(logp + logdetJ) 
      for(int i = 0; i < 24; ++i){
        meanGradient(i) += (gradientP(i, s) + gradientJ(i) )/ S;
      }
      LB(iter) += (logPeval(s) + logJeval - logQeval)/S;
    }
    for(int i = 0; i < 24; ++i){
