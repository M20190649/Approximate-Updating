...
double pLogDensMix(mat data, vec theta, mat mean, cube Linv, vec dets, vec weights){
  int N = data.n_rows;
  int K = weights.n_elem;
  // Constrained Positive
...
// [[Rcpp::export]]
Rcpp::List scoreDeriv(mat data, Rcpp::NumericMatrix lambdaIn, vec theta, int K, mat mean, cube Linv, vec dets, vec weights){
  Map<MatrixXd> lambda(Rcpp::as<Map<MatrixXd> >(lambdaIn));
  double qEval;
