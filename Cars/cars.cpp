...
struct logP {
  ...
  T operator ()(const Matrix<T, Dynamic, 1>& theta) // derivative is with respect to theta (with xT+1 included), so theta matrix is the operator () input.
    ...
    T prior = -(hyperParams(0) + 1) * log(sigSqX)  -  hyperParams(1) / sigSqX  - 
      ...
      pow(delta - hyperParams(6), 2) / (2*hyperParams(7));
    ...
    for(int t = 1; t < obs; ++t){
      ...
      omegaSum = 0;
      for(int k = 0; k < N; ++k){
        T z = sqrt(1.0 / theta(5 + t*N + k)) * abs(randn<vec>(1)[0]);
        T skewT = delta * z  +  sqrt(sigSqE / theta(5 + t*N + k)) * randn<vec>(1)[0];
        vNew(k) = vResample(k)  +  skewT; // Step Ahead transition
    ...
    for(int k = 0; k < N; ++k){
      ...
      T densPart2 = pow(1 + pow(VTplus1 - vNew(k), 2) / (nu*(sigSqE + pow(delta, 2))), -(nu+1)/2);
      T VMean = delta / (sigSqE + pow(delta, 2)) * (VTplus1 - vNew(k));
      T Vscale = sqrt((nu + pow(VTplus1 - vNew(k), 2)*(sigSqE + pow(delta, 2))) / (nu + 1) * (1.0 - pow(delta, 2) / (sigSqE + pow(delta, 2))));
      T VCDF = 0;
      for(int v = 0; v < 501; ++v){ // Evaluate from v = 0 to v = 5
...
double evalLogQ (Matrix<double, Dynamic, 1> epsilon, Matrix<double, Dynamic, 1> lambda){
  double logEpsDens = 0;
  double logDetJ = lambda(0) + lambda(5)*epsilon(0);
  for(int i = 0; i < 5; ++i){
...
Matrix<double, Dynamic, 1> QDeriv(Matrix<double, Dynamic, 1> epsilon, Matrix<double, Dynamic, 1>  lambda){
  ...
  output(5) = - (epsilon(0) + 1.0/lambda(5)); //U_11
  output(10) = - epsilon(0); // U_21
  output(11) = - (epsilon(1) + 1.0/lambda(11)); // U_22
  output(15) = - epsilon(0); // U_31
  output(16) = - epsilon(1); //U_32
  output(17) = - (epsilon(2) + 1.0 / lambda(17)); //U_33
  output(23) = - 1.0 / lambda(23); //U_44
  output(29) = - 1.0 / lambda(29); //U_55
  return output;
}
...
Matrix<double, Dynamic, 1> FDeriv (Matrix<double, Dynamic, 1> epsilon, Matrix<double, Dynamic, 1>  lambda){
  ...
  output(0) = exp(lambda(0) + lambda(5)*epsilon(0));
  output(1) = exp(lambda(1) + lambda(10)*epsilon(0) + lambda(11)*epsilon(1));
  output(2) = exp(lambda(2) + lambda(15)*epsilon(0) + lambda(16)*epsilon(1) + lambda(17)*epsilon(2));
  output(3) = output(4) = 1;
  output(5) = epsilon(0) * output(0);
  output(10) = epsilon(0) * output(1);
  output(11) = epsilon(1) * output(1);
  output(15) = epsilon(0) * output(2);
  output(16) = epsilon(1) * output(2);
  output(17) = epsilon(2) * output(2);
  output(20) = output(25) = epsilon(0);
  output(21) = output(26) = epsilon(1);
  output(22) = output(27) = epsilon(2);
  output(23) = output(28) = epsilon(3);
  output(29) = epsilon(4);
  return output;
}
  ...
  while(diff > threshold){
    ...
    if(iter == 1 | checkIS > thresholdIS){
      ...
      for(int s = 0; s < S; ++s){
        ...
        impliedEpsilon(1) = (log(theta(1, s)) - lambda(1) - lambda(5) * impliedEpsilon(0)) / lambda(6);
        impliedEpsilon(2) = (log(theta(2, s)) - lambda(2) - lambda(10) * impliedEpsilon(0) - lambda(11) * impliedEpsilon(1)) / lambda(12);
        for(int i = 3; i < 5; ++i){
