...
\begin{figure}[hb]
\centering
\includegraphics[width = 0.6\textwidth]{basicFit.png}
\end{figure}
...
This formulation removes $\gamma$ from $\theta$, but adds five additional parameters, $\zeta, \alpha_0, \beta_0, \alpha_1$, and $\beta_1$, each with a $N(\mu_i, \tau_i^2)$ prior. The likelihood can be calculated by the Hamilton filter, and is given by
\begin{equation}
...
\end{align}
The recursion in (\ref{HF}) is initialised at $\xi_3 = \{0.9, 0.1\}$.
\end{document}
