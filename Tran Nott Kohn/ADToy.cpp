...
// evaluate and automatially differentiate log(p(y_1:T | theta)) wrt theta
struct logP {
  ...
  T operator ()(const Matrix<T, Dynamic, 1>& theta) // derivative is with respect to theta, so theta matrix is the operator () input.
    const{
    ...
    T prior = - 2 * log(sigSqV)  -  1 / sigSqV  -  2 * log(sigSqW)  -  1 / sigSqW;
    ...
    T yAllDens, ytDens, omegaSum;
    // yAllDens = log(p(y_1:T | theta))
    // ytDens = log(p(y_t | theta))
    // omegaSum = sum of omega
    
    // Sample x1 from stationary distribution
    for(int k = 0; k < N; ++k){
      xOld(k) = sqrt(5) * noise(k, 0); 
    }
    yAllDens = 0;
    ...
    for(int t = 1; t < obs; ++t){
      ...
      omegaSum = 0;
      for(int k = 0; k < N; ++k){
        xNew(k) = xResample(k)/2  +  25 * xResample(k) / (1 + pow(xResample(k), 2))  + 
          8 * cos(1.2*(t+1))  +  sqrt(sigSqV) * noise(k, t); // step ahead transition
        omega(k) = 1.0 / sqrt(2*3.14159*sigSqW) * exp(-pow(y(t) - pow(xNew(k), 2)/20, 2) / (2*sigSqW)); // y ~ N(x^2/20, sigSqV)
    ...
    return yAllDens + prior;
  }
};

// evaluate log(q(theta, xT+1)) = log(q(eps)) - log(det(jacobian))
double evalLogQ (Matrix<double, Dynamic, 1> epsilon, MatrixXd lambda){
  double logEpsDens = -0.5 * (pow(epsilon(0), 2) + pow(epsilon(1), 2));
  double logDetJ = lambda(0) + lambda(2)*epsilon(0)  +  lambda(1) + lambda(4) * epsilon(1) +
    log(lambda(2)) + log(lambda(4)); 
...
Matrix<double, Dynamic, 1> QDeriv(Matrix<double, Dynamic, 1> epsilon, MatrixXd lambda){
  Matrix<double, Dynamic, 1> output(5); 
  output(0) = - 1; //mu_1
  output(1) = - 1;
  output(2) = - (epsilon(0) + 1.0/lambda(2));
  output(3) = 0;
  output(4) = - (epsilon(1) + 1.0/lambda(4)); 
  return output;
}

// d log theta / d lambda
Matrix<double, Dynamic, 1> FDeriv (Matrix<double, Dynamic, 1> epsilon, MatrixXd lambda){
  Matrix<double, Dynamic, 1> output(5);
  output(0) = exp(lambda(0) + lambda(2)*epsilon(0));
  output(1) = exp(lambda(1) + lambda(3)*epsilon(0) + lambda(4)*epsilon(1));
  output(2) = epsilon(0) * output(0);
  output(3) = epsilon(0) * output(1);
  output(4) = epsilon(1) * output(1);
  return output;
}
...
// Main VB algorithm
// [[Rcpp::export]]
Rcpp::List VBIL_Toy (Rcpp::NumericMatrix yIn, Rcpp::NumericMatrix lambdaIn, int S, int N = 100, int maxIter = 5000,
  ...
  Matrix<double, Dynamic, 1> gradientQ(5), gradientF(5), meanGradient(5), meanGradientSq(25), 
  Mt(5), Vt(5), LB(maxIter+1), logPeval(S), gradP(2);
  MatrixXd theta(2, S), gradientP(5, S), epsilon(2, S);
  ...
  mat sobol = sobol_points(S+100, 2);
  ...
  while(diff > threshold){
    ...
    if(iter == 1 | checkIS > thresholdIS){
      ...
      for(int s = 0; s < S; ++s){
        ...
        theta(0, s) = exp(lambda(0) + lambda(2) * epsilon(0));
        theta(1, s) = exp(lambda(1) + lambda(3) * epsilon(1) + lambda(4) * epsilon(2));
        // generate standard normal noise for the particle filter, convert to Eigen format
        Rcpp::NumericMatrix noiseRcpp(N, T);
        for(int t = 0; t < T; ++t){
          noiseRcpp(Rcpp::_, t) = Rcpp::rnorm(N);
        }
        ...
        for(int i = 0; i < 5; ++i){
          ...
          if(i == 0 | i == 2){
            gradientP(i, s) = gradP(0);
          } else {
            gradientP(i, s) = gradP(1);
          }
          //update gradients
      ...
      for(int s = 0; s < S; ++s){
        ...
        Matrix<double, Dynamic, 1> impliedEpsilon(2);
        impliedEpsilon(0) = log(theta(0, s)) - lambda(2) * epsilon(0);
        impliedEpsilon(1) = log(theta(1, s)) - lambda(3) * epsilon(0) - lambda(4) * epsilon(0);
    ...
    // adam updating rule for lambda values - changed to adagrad for now
    for(int i = 0; i < 5; ++i){
