...
struct logP {
  ...
  T operator ()(const Matrix<T, Dynamic, 1>& theta) // derivative is with respect to theta (with xT+1 included), so theta matrix is the operator () input.
    ...
    T yAllDens, ytDens, omegaSum, xTS1Dens;
    // yAllDens = log(p(y_T+1:T+S | theta))
    // ytDens = log(p(y_t | theta))
    // omegaSum = sum of omega
    // XTplusS1Dens = log(p(x_T+S+1 | theta, y_1:T+S))
    yAllDens = 0;
    
    // Sample x_T+1 from proposal distribution
    for(int k = 0; k < N; ++k){
      xNew(k) = conMean + sqrt(conVar) * noise(k, 0); 
    }
    // calculate weights
    omegaSum = 0;
    for(int k = 0; k < N; ++k){
      T var = exp(xNew(k)); // variance of y
      T yDensity =  1.0 / sqrt(2*3.14159*var) * exp(-pow(y(0), 2) / (2*var));
      omega(k) = yDensity;
    ...
    yAllDens += ytDens;
    // push xNew to xOld as we step from T+1 to T+2
    for(int k = 0; k < N; ++k){
      xOld(k) = xNew(k);
    }
    ...
    for(int k = 0; k < N; ++k){
      xTS1Dens += pi(k) / sqrt(2*3.14159*sigSq) * exp(-pow(xTS1 - (mu + phi*(xNew(k)-mu)), 2) / (2*sigSq));
    }
    // interested in log density
    xTS1Dens = log(xTS1Dens);
    return yAllDens + xTS1Dens + prior;
  }
};
...
Rcpp::List VBIL_PF_Update (Rcpp::NumericMatrix yIn, Rcpp::NumericMatrix lambdaIn, Rcpp::NumericMatrix uDiagIn,
  ...
  double logQeval;
  int T = y.rows();
  // Calculate conditional distribution for q(x_T+1 | theta)
  ...
  while(diff > threshold){
    ...
    if(iter == 1 | checkIS > thresholdIS){
      ...
      for(int s = 0; s < S; ++s){
        ...
        // Put these draws into XT+1 Condtional Distribution - could be vectorized but log(sigSq) is slightly annoying.
        double conMean = priorMu(3)  +  
  ...
  Map<MatrixXd> Uout(lambda.bottomRows(16).data(), 4, 4);
