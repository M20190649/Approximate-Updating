...
Methods:
\begin{itemize}
\item PMMH \citep{Andrieu2010}: 20,000 draws, first 10,000 discarded. $\theta^{(i)} \sim MVN(\theta^{(i-1)}, 0.05)$ proposal distribution. Sample $\{\mu, \phi, \sigma^2, x_{0:T} \}$ jointly, then use all draws to estimate $\hat{p}(x_{T+1}) \approx \sum_{i=1}^10000 p(x_{T+1} | x_T^{(i)}, \mu^{(i)}, \phi^{(i)}, \sigma^{2,(i)}) / 10000$.
...
\indcludegraphics{standardDev}
\end{subfigure}
\label{simulation}
\caption{Simulation Posterior Means and Standard Deviations for twenty datasets}
\end{figure}
