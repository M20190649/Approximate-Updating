...
\begin{algorithm}[H]
 ...
 \While{$\mathcal{L}(q, y_{1:T})^{(m)} - \mathcal{L}(q, y_{1:T})^{(m-1)} > \epsilon$}{
  Set $\tilde{\gamma}^{(m)}$ using \ref{mf9}.\;
  Set $\tilde{\tau}^{(m)}$ using \ref{mf10}.\;
  Set $\tilde{\beta}^{(m)}$ using \ref{m11}.\;
...
% no need to say more than this here.
% But actually, I think you should just do the very simple Normal Inverse Gamma model here, since that has the parallel with VB below. Then in your example later on, you can do the full AR(2) example. I think this will help a lot.
...
\end{equation}
% Changed the matrices above to align properly
$\gamma(k)$ is the $k-th$ autocorrelation and $\textbf{y} = (y_1, y_2)'$.
% You also need to mention what $p(y_1, y_2 | \theta)$ is, and perhaps something about how $p(y_t | y_{1:t-1}, \theta)$ is available from the assumptions regarding the distribution of $\epsilon_t$.
% Also, I don't like the use of vector notation (for $y=(y_1,y_2)$ )and subscripts for the remaining $y$'s in the likelihood. You can just break the equation over two lines if needed. 
% Finally, perhaps you could say something about where $\gamma_0$ and $\gamma_1$ come from, or at least provide a reference for the reader to go and find out more about this model so you don't have to go into too much detail here.

{\bf We assume that the prior distribution, is given by }
% remember Tas's comment about stating that the joint prior is assumed to be the product of independent marginal priors
...
{\bf Upon examination of the form of the product of the likelihood function in (\ref{likelihood}) and the prior density in (),}
	%need to be able to reference the prior too
{\bf it can be seen that the full} conditional distribution $p(\sigma^2 | y, \phi_1, \phi_2)$ is {\bf an} Inverse Gamma {\bf distribution},
% can you say which one? Be sure it is clear how to interpret the parameters of this distribution too.
 while the conditionals for both $\phi$ parameters are intractable and sampled using a Metropolis-Hastings step 
% Since you are actually using an M-H step, you should have told the reader what this is back up when you introduced the Gibbs sampler.
with truncated {\bf bivariate} %?
normal random walk {proposal distribution}. The truncation ensures that draws {\bf of $(\rho_1,\rho_2)$} %just to be clear
are only taken from the AR(2) stationary region.

%Any graph that you put in to the document needs to be clearly explained. Why discard any? What does 'thinned' mean, and why did you do it? etc.... Better to put the details of the MCMC in a paragraph, and then point to the figure, with only the essential details left in the caption. 
% Also, ideally you would be able to put the Greek symbols in the margins of the graphs, rather than 'Sigmasquared', for example...
