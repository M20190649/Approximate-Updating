...
\begin{frame}
\section{Bayesian Computation Techniques}
A popular technique is to sample the posterior distribution with Markov Chain Monte Carlo (MCMC). MCMC:
\begin{itemize}
\item Converges to the true posterior.
\item Can be very slow, and is not parallelisable.
...
\item Has no theoretical guarantees to be close to the truth.
\end{itemize}
\end{frame}
...
\end{equation}
This is non-negative and equals zero if and only if $p(\boldsymbol{\theta} | y_{1:T}) = q(\boldsymbol{\theta} | \boldsymbol{\lambda})$ almost everywhere.
\end{itemize}
\end{frame}
...
(\ref{mf2}) is applied to the Normal-Inverse Gamma model to find $q(\mu | \boldsymbol{\lambda}_1)$.
\begin{itemize}
\item Step One: Write the full log joint distribution,
\begin{align}
\log(p(y_{1:T}, \mu, \sigma^2)) &= -(T/2 + \alpha + 2)\log(\sigma^2) - \frac{\sum_{t}(y_t - \mu)^2}{2\sigma^2} \nonumber \\
