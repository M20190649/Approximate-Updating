...
\begin{itemize}
\item There is an increasing demand for forecasts to be of the entire predictive density of the variable of interest.
...
\item The algorithm use should scale to multivariate observations driven by complex models with large datasets.
\end{itemize}
\end{frame}
