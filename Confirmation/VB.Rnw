...
\begin{figure}[hb]
<<klnorm, echo = FALSE, fig.width = 6, fig.height = 2>>=
library(ggplot2)
xseq = seq(-0.5, 1, 0.01)
w1 = dnorm(xseq, 0, 0.1)
w2 = dnorm(xseq, 0.5, 0.1)
ggplot() + geom_line(data=NULL, aes(x=xseq, y = w1), colour = "red") + geom_line(data=NULL, aes(x=xseq, y = w2), colour = "green") + labs(x="Support", y = "Density") + theme_bw()
@
...
\label{AR2MCMC}
\centering
<<AR2MCMC, echo = FALSE, fig.height = 4, fig.width = 4>>=
library(ggplot2)
library(gridExtra)
draws <- read.csv("AR2thin.csv")[,-1]
colnames(draws) = c("Phi1", "Phi2", "SigmaSquared")
m1 <- ggplot(data=draws, aes(x=Phi1)) + geom_density() + theme_bw() + ylab(NULL) + xlab(expression(phi[1]))
m2 <-  ggplot(data=draws, aes(x=Phi2)) + geom_density() + theme_bw() + ylab(NULL) + xlab(expression(phi[2]))
m3 <-  ggplot(data=draws, aes(x=SigmaSquared)) + geom_density() + theme_bw() + ylab(NULL) + xlab(expression(sigma^2))
j1 <- ggplot(data=draws, aes(x=Phi1, y = Phi2)) + geom_density2d() + theme_bw() + xlab(expression(phi[1])) + ylab(expression(phi[2]))
j2 <- ggplot(data=draws, aes(x=Phi1, y = SigmaSquared)) + geom_density2d() + theme_bw() + xlab(expression(phi[1])) + ylab(expression(sigma^2))
j3 <- ggplot(data=draws, aes(x=Phi2, y = SigmaSquared)) + geom_density2d() + theme_bw() + xlab(expression(phi[2])) + ylab(expression(sigma^2))
grid.arrange(m1, m2, m3, j1, j2, j3, ncol = 3)
@
...
\centering
\label{AR2results}
<<AR2summary, echo = FALSE>>=
library(knitr)
summary = matrix(c(0.83, 0.10, 0.04, 0, 0.05, 50, 79.7, -77.7, "1.3 x 10^{-6}", 
...
colnames(summary) = c("mu1", "mu2", "L11", "L21", "L22", "alpha", "beta", "ELBO", "Runtime (seconds)")
rownames(summary) = c("Mean Field", "SGA", "SGA-RP", "SGA-CV")
kable(summary)
@
