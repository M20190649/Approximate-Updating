...
MCMCres$Method = 'MCMC'
VBres$Method = 'VB-Non-Diag'
MFres$Method = 'VB-Diag'
T100S10df = rbind(MCMCres, VBres, MFres) %>% gather(variable, value, -Method)
T100S10df$variable = factor(T100S10df$variable, levels=c('lnSigXSq', 'lnSigYSq', 'Phi', 'Gamma', 'X_T', 'X_T+S', 'S+h step Logscore',
                                                       'h step Filtered Logscore', 'h step refit Logscore'))
T100S10df$Method = factor(T100S10df$Method, levels = c('MCMC', 'VB-Diag', 'VB-Non-Diag'))

p1 = T100S10df[as.integer(T100S10df$variable) <= 6,] %>%
  ggplot() + geom_boxplot(aes(y=value, x=Method)) + facet_wrap(~variable, scales='free') +
  labs(x=NULL,y=NULL)
p2 = T100S10df[as.integer(T100S10df$variable) >= 7,] %>%
  ggplot() + geom_boxplot(aes(y=value, x=Method)) + facet_wrap(~variable) + 
  labs(y=NULL)
gridExtra::grid.arrange(p1, p2, ncol=1, layout_matrix=matrix(c(1,1,2),ncol=1))
