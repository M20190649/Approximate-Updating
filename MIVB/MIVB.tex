...
Many time series models of interest contain a set of global parameters $\boldsymbol{\theta}$ and observation specific latent variables $x_{0:T}$ such that $x_t$ is assumed to follow a Markovian structure and $y_t$ is conditionally independent of $y_{1:{t-1}$, given $\boldsymbol{\theta}$ and $x_t$. One of the most common computational techniques for models with latent states is Markov Chain Monte Carlo (MCMC), which involves iterative sampling from a Markov Chain designed to converge to the true posterior distribution. MCMC can have slow convergence rates and generally does not admit parallel processing or data subsampling. In this paper it is assumed that an MCMC sample of $p(\boldsymbol{\theta}, x_{0:T} | y_{1:T})$ is available at time $T+S$, but convergence of MCMC is slow, so a posterior distribution conditioned on any additional observations of $y_t, t = T+1, \dots, T+S$ will not be available before observation of $y_{T+S+h}$. 

With $p(\boldsymbol{\theta}, x_{0:T}, y_{1:T})$ the forecast distribution of p(y_{T+S+h} | y_{T})$ is available, however incorporating the information contained in realisations of $y_{T+1:T+S}$ to obtain $p(y_{T+S+h} | y_{T+S})$ is desirable, as it should provide a forecast with reduced uncertainty. The addition of $y_{T+1:T+S}$ is used to update the conditional posterior distribution of $p(x_{T+S+h} | \boldsymbol{\theta}, y_{1:T})$ to $p(x_{T+S+h} | \boldsymbol{\theta}, y_{1:{T+S})$, which can be performed with filtering techniques such as the Kalman filter with certain restrictions on the model. If these restrictions are not met, or the filtering technique is too computationally intensive to perform before $T+S+h$, then the forecaster is unable to obtain $p(y_{T+S+h} | y_{T+S})$. 
...
\begin{algorithm}[H]
 ...
 \Input{Observations of $\boldsymbol{\theta}, x_{T-V:T}$}
