...
Rcpp::List SGA_DLM(vec y, int M, int maxIter, vec Mu, mat L, bool meanfield=false, bool xderiv=true,
  bool variance=true, double threshold=0.005, double alpha=0.05, double beta1=0.9, double beta2=0.999){
  double e = pow(10, -8);
  int T = y.n_elem;
  ...
  while(diff > threshold | iter < 100){
    ...
    for(int m = 0; m < M; ++m){
      ...
      double sigmaSqY = exp(transf[0]);
      double sigmaSqX = exp(transf[1]);
      vec dpdt = pDeriv(y, x, sigmaSqY, sigmaSqX, transf[2], transf[3], xderiv);
      vec dmu = muDeriv(dpdt, sigmaSqY, sigmaSqX, T);
      pMu += dmu/M;
      pMuSq += pow(dmu, 2)/M;
      if(variance){
        vec eps = epsilon.col(m);
        mat dl = LDeriv(dpdt, sigmaSqY, sigmaSqX, eps, L, T, meanfield, xderiv);
        pL += dl/M;
        pLSq += pow(dl, 2)/M;
      }
    }
